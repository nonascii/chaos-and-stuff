...
#                                      (self._x * self._y) - (1.0001* self._z) 
    #    self._x , self._y, self._z = self._x + random.randint(-1,1), self._y + random.randint(-1,1), self._z + random.randint(-1,1) 
        ...
        self._x , self._y, self._z = self._x + 0.5 * (self._y + self._z) * (dt * dt),\
                                     self._y - 0.5 * (self._x -self._z )* (dt * dt) *math.sin(self._y/6.28*dt),\
                                     self._z - 0.5 * (self._x + self._y) *(dt*dt)
    ...
    while True:
        ...
        for x in range(-200, 200, 50): 
            for y in range(-200,200, 50):
                for z in range(-200,200,50):
                    for w in range(-200,200,50):
                        if math.sqrt(math.fabs(x*x + y*y + z*z +w*w)) == 200: 
...
#            if age >= 1:
#                break
            glRotatef(math.cos(1/age), 90, 180,270)
