...
class Point:
    ...
    def swap(self,dt):
        self._x , self._y, self._z = self._x + ( -1*(self._y*self._y) -1*(self._z*self._z) + 0.2 *(-self._x + 8)) * (dt) ,\
                                     self._y + ( (self._x*self._y) -4*(self._x*self._z) -self._y + 1   ) * (dt),\
                                     self._z + ( (4*self._x*self._y) + (self._x * self._z) -self._z    ) * (dt)
            ...
            for y in range(-20, 20, 2):
                for z in range(-20, 20, 2):
                    if math.sqrt(math.fabs(x*x + y*y + z*z)) <= 100: 
                     points.append(Point(x/10.0,y/10.0,z/10.0))
